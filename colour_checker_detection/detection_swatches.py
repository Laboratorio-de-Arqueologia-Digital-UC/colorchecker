...
def main():
    ...
    # BUSCAR SOLO 1 IMAGEN (La primera CR2/ARW)
    img_files = list(images_dir.glob("*.CR2")) + list(images_dir.glob("*.ARW"))
    if not img_files:
        LOGGER.error("No se encontraron imagenes .CR2 o .ARW en %s", images_dir)
        return
        
    img_path = img_files[0] # PROBAR PRIMERA IMAGEN
    ...
    if not detections:
        LOGGER.warning("No se detectó nada. Abortando.")
        return
    ...
    # Al pasar srgb_refs, sample_colour_checker rota el quad internamente si mejora MSE
    ...
    if linear_data:
        ...
        LOGGER.info("Stats Lineales:")
        LOGGER.info("  Brightest Patch Index: %d (Ideal: 18/19)", brightest_idx)
        LOGGER.info("  Valor White (Index 18): %s", np.round(vals[18], 4))
        LOGGER.info("  Valor Black (Index 23): %s", np.round(vals[23], 4))
        
        # Plot
        fig, ax = plt.subplots(1, 2, figsize=(12, 6))
        ax[0].imshow(np.clip(img_srgb, 0, 1))
        ax[0].set_title("Detección (sRGB) + Quad Optim")
        
        # Dibujar Quad
        poly = quad_optimized.astype(int)
        poly = np.vstack([poly, poly[0]]) # Cerrar
        ax[0].plot(poly[:,0], poly[:,1], 'r-', linewidth=2)
        ax[0].text(poly[0,0], poly[0,1], "0", color='yellow', fontsize=12, fontweight='bold')
        
        # --- VISUALIZATION LOGIC (Uses Optimized Quad) ---
        try:
            # Project swatch centers using the OPTIMIZED quad (same as extraction)
            # This ensures visual indices match extracted values
        ...
        for i in range(24):
             ...
             # Using the VISUALIZED color for contrast decision
             vis_lum = np.mean(swatch_grid[r, c])
             txt_col = 'black' if vis_lum > 0.5 else 'white'
             
             # Format: Index (0..23)
             lbl = str(i)
             ax[1].text(c, r, lbl, ha='center', va='center', fontsize=10, color=txt_col, fontweight='bold')
        
        out_path = output_dir / "debug_detection.png"

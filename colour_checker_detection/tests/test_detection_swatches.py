...
def test_process_image_geometry_and_extraction(tmp_path):
    ...
    assert len(results) > 0, (
        "No methods detected anything (could be unexpected for these test images)"
    )
    ...
    for method, data in results.items():
        ...
        assert quad.shape == (4, 2), (
            f"Method {method} quad shape mismatch: {quad.shape}"
        )
        ...
        assert swatches.shape == (24, 3), (
            f"Method {method} swatches shape mismatch: {swatches.shape}"
        )

        # ✅ Que valores RGB estén en rango válido
        # Linear raw reading implies 0-1 range (as per read_raw_high_res implementation)
        assert np.all(swatches >= 0) and np.all(swatches <= 1.5), (
            f"Method {method} RGB out of range 0-1.5"
        )
